...
FILE *student_file, *updata_student_file;
...
FIFO_Status_st student_sys_init(FIFO_Buf_st * students_queue, Item * item, int lenght);
{
		  ...
		  students_queue->head = students_queue->;
...
{ 
	...
	void add_student_manualy(FIFO_Buf_st *student_queue)
	{
		Item new_student;
	    int i;
